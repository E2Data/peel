...
class Yarn3(
  ...
  override protected def logFilePatterns(): Seq[Regex] = {
    ...
    config.getStringList(s"system.$configKey.config.workers").asScala.map(Pattern.quote).map(slave =>
      s"hadoop-$user-resourcemanager-$slave\\.log".r)
  }
  ...
  override def configuration() = SystemConfig(config, {
    ...
    List(
      ...
      SystemConfig.Entry[Model.GenericKeyValuePairs](s"system.$configKey.config.yarn", s"$conf/yarn-site.xml", templatePath("conf/site.xml"), mc),
      ...
      SystemConfig.Entry[Model.GenericKeyValuePairs](s"system.$configKey.config.resource-types", s"$conf/resource-types.xml", templatePath("conf/site.xml"), mc),
      SystemConfig.Entry[Model.NamedKeyValuePairs](s"system.$configKey.config.capacity-scheduler", s"$conf/capacity-scheduler.xml", templatePath("conf/capacity-scheduler.xml"), mc),
  ...
  override def start(): Unit = {
    ...
    while(!isUp) {
      try {
        val total = config.getStringList(s"system.$configKey.config.workers").size()
        // yarn does not reset the resourcemanagers log at startup
        val init = Integer.parseInt((shell !! s"""cat $logDir/hadoop-$user-resourcemanager-*.log | grep 'registered with capability:' | wc -l""").trim())
        ...
        while (curr - init < total) {
          ...
          // get new values
          curr = Integer.parseInt((shell !! s"""cat $logDir/hadoop-$user-resourcemanager-*.log | grep 'registered with capability:' | wc -l""").trim())
  ...
  def isRunning = {
    (shell ! s""" ps -p `cat ${config.getString(s"system.$configKey.config.env.HADOOP_PID_DIR")}/hadoop-*-resourcemanager.pid` """) == 0 ||
    (shell ! s""" ps -p `cat ${config.getString(s"system.$configKey.config.env.HADOOP_PID_DIR")}/hadoop-*-nodemanager.pid` """) == 0
  }
}
